...
def basicProfile(request):
    ...
    if facility_profile_level == 'Brand':
        ...
        if request.session['user_type'] == "Personal Training Company":
            ...
            trainlocation = data.get("tlocation")
        ...
        city = data.get("city")
        if request.session['user_type'] == "Personal Training Company":
            if own_facility == 'on' or own_facility==0:
                country = None
                city =  None
                location= ''
            if country =='':
...
def mediaSocialMedia(request):
    ...
    if auth.user_type == "Gym or Studio":
        if data['virtual_tourold'] == '' and virtual_tour != '' :
            ...
            # //update
    try:
        isProfile = UserMediaRecord.objects.filter(user_id=user_id, type='ProfilePhoto').values()
        isProfile = isProfile[0]['data']
    except:
        isProfile = ''
    ...
    context = {
        0:isUpdateTile,1:isUpdateVirtual,2:isUpdateProfile,3:virName,4:isProfile
        }
...
def offerSubmit(request):
    ...
    if promotion_type == 1:
        ...
        quantity_free= data['quantity_free']
        if data['avail_limit'] == '':
            avail_limit= 0
