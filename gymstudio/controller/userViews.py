...
def profile(request):
    ...
    bsReacord = BasicInfo.objects.all().exclude(Q(user_id=user_id)|Q(user__user_type="Gym or Studio")).values_list('branch_id',flat=True)
    ...
    # Branch.objects.all().values_list('id','branch_name').order_by('branch_name')
...
def includedList1(request):
    user_id = request.session['user_id']
    awardInfo =UserAmenity.objects.all().filter(user_id=user_id).values_list('amenity_id',flat=True)
    awardList = Amenity.objects.all().exclude(~Q(id__in = list(awardInfo))).values_list('id','name').order_by('name')
    context = {
        # 'users': user,
        'includeList': awardList
    }

    return render(request, 'elements/includeList1.html', context)

def includedList2(request):
    user_id = request.session['user_id']
    awardInfo =UserAmenity.objects.all().filter(user_id=user_id).values_list('amenity_id',flat=True)
    awardList = Amenity.objects.all().exclude(~Q(id__in = list(awardInfo))).values_list('id','name').order_by('name')
    context = {
    ...
    return render(request, 'elements/includeList2.html', context)
