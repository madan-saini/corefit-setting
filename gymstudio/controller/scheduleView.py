...
def calendar(request):
    ...
    title = settings.SITE_TITLE + ' | ' + 'Calendars'
...
def getSEventData(request):
    ...
    availAllDays = availability.all().filter(user_id=user_id).filter(con1 & con2 & con3).values()
    # print(availAllDays)
            ...
            if avail['type_of_services'] == 3:
                ...
                if recordd != None:
                    ...
                    if user.get(id=recordd['type_of_amenity']):
                        ...
                        dataResults[count].append(str(ddStart))
                        dataResults[count].append(str(ddEnd))
                        dataResults[count].append(str(date.datetime.strftime(date_obj, '%I:%M%p'))+' '+auth['name'])    
                ...
                if recordd:
                    dataResults[count].append(str(ddStart))
                    dataResults[count].append(str(ddEnd))
                    dataResults[count].append(str(date.datetime.strftime(date_obj, '%I:%M%p'))+' '+str(recordd['session_name'])) 
                    dataResults[count].append(str(avail['date_of_booking']))
                    dataResults[count].append(str(avail['id']))
