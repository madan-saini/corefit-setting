...
def profile(request):
    ...
    if bool(User.objects.filter(freelance_id=auth.id).filter(~Q(user_id = ''))):
        empDe = User.objects.filter(freelance_id=auth.id).filter(~Q(user_id = ''))

    if bool(Request.objects.filter(freelancer_user_id=auth.id,status=0)):
        isRequest = Request.objects.filter(freelancer_user_id=auth.id,status=0).first()
